...
2. **Install theme components**
  ```shell
  # @ roots-starter-wp.com/site/web/app/themes/sage
  $ npm install
  $ bower install
  $ gulp
  ```
...
3. **Fire up the server** (be patient, but watch the console––it may prompt for your system password)
  ```shell
  # @ roots-starter-wp.com/trellis
  $ vagrant up
  ```
  _Note: to shut down the server:_ `vagrant halt`

4. **Test the install** at [roots-starter-wp.test](http://roots-starter-wp.test/)
...
```shell
# @ roots-starter-wp.com/trellis
./deploy.sh <environment> roots-starter-wp.com

# OR
ansible-playbook deploy.yml -e "site=roots-starter-wp.com env=<environment>"
```

**To rollback a deploy:**
```shell
ansible-playbook rollback.yml -e "site=roots-starter-wp.com env=<environment>"
```

## Theme development

In **development**, run gulp in _watch_ mode for live updates at [localhost:3000](http://localhost:3000). **Important**: always use the [roots-starter-wp.test](http://roots-starter-wp.test/wp/wp-admin/) URL to access the WordPress admin.
```shell
# @ roots-starter-wp.com/site/web/app/themes/sage
$ gulp watch
```
